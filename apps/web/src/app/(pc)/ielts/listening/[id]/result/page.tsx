...
export default function IeltsListeningResultPage() {
  ...
  if (!result) return <div className="text-center py-16 text-red-500">Result not found</div>;
  ...
  return (
    <div className="max-w-3xl mx-auto">
      <div className="text-center py-10">
        ...
        <p className="text-sm text-slate-400 mt-1">{result.totalScore} / {result.maxScore} correct</p>
      </div>
          ...
          <div key={idx} className="border rounded-xl overflow-hidden">
            <button onClick={() => setExpandedSection(expandedSection === idx ? null : idx)}
              ...
              <div>
                ...
                <p className="text-sm text-slate-500">{s.correctCount}/{s.totalCount} correct · {s.score}/{s.maxScore} points</p>
              </div>
                ...
                {s.questions.map((q) => (
                  <div key={q.questionId} className="px-5 py-3">
                    ...
                    <div className="flex gap-4 text-sm">
                      <span className={q.isCorrect ? 'text-green-600' : 'text-red-600'}>Your answer: {q.userAnswer || '(Not answered)'}</span>
                      {!q.isCorrect && <span className="text-green-600">Correct answer: {q.correctAnswer}</span>}
                    </div>
                  </div>
                ))}
              </div>
      ...
      <div className="text-center">
        <Link href="/ielts" className="text-primary-600 hover:underline text-sm">← Back to exam list</Link>
      </div>
    </div>
  );
}
