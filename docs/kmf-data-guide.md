...
- **子题型**: 404（不带内容的填空题）
- **content 格式**: 两种格式，取决于剑桥版本
  - **旧格式 (C18-)**: BBCode + HTML table
    - `[center][b]Title[/b][/center]` → 标题
    - `[b]Subtitle[/b]` → 子标题
    - `● text [blank]1[/blank]` → 填空行
    - `<table>...</table>` → 表格
  - **新格式 (C17+)**: 纯 HTML 段落（`<p>`）
    - `<p style="text-align:center;"><strong>Title</strong></p>` → 标题
    - `<p><strong>SubTitle</strong></p>` → 子标题
    - `<p>● text [blank]1[/blank]</p>` → 填空行
    - `<table>...</table>` → 表格（不变）
    - **⚠️ HTML `<p>` 标签可能跨多行**，需先合并再解析
  - **统一到 NoteModeGroup passageText 格式**:
    - 主标题 → `# Title`（居中加粗大字）
    - 子标题 → `## SubTitle`（加粗小字）
    - 表格 → `[table] Title\n| col |`（管道格式）
    - 填空 → `{QX}` 格式（非 `(N) ______`！）
